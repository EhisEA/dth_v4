...
run-dev:
	$(FLUTTER) run --flavor dev -t $(MAIN_DEV)

run-prod:
	$(FLUTTER) run --flavor prod -t $(MAIN_PROD)

build-apk-dev:
	$(FLUTTER) build apk --flavor dev -t $(MAIN_DEV)

build-apk-prod:
	$(FLUTTER) build apk --flavor prod -t $(MAIN_PROD)

build-aab-dev:
	$(FLUTTER) build appbundle --flavor dev -t $(MAIN_DEV)

build-aab-prod:
	$(FLUTTER) build appbundle --flavor prod -t $(MAIN_PROD)

build-ios-dev:
	$(FLUTTER) build ios --debug --flavor dev -t $(MAIN_DEV)

build-ios-prod:
	$(FLUTTER) build ios --debug --flavor prod -t $(MAIN_PROD)

build-ipa-dev:
	$(FLUTTER) build ipa --flavor dev -t $(MAIN_DEV)

build-ipa-prod:
	$(FLUTTER) build ipa --flavor prod -t $(MAIN_PROD)

analyze-app:
	$(FLUTTER) build apk --flavor dev -t $(MAIN_DEV) --analyze-size --target-platform=android-arm64

run-app:
	@read -p "Enter FLAVOR (dev or prod): " FLAVOR; \
	case $$FLAVOR in \
		dev) TARGET="$(MAIN_DEV)" ;; \
		prod) TARGET="$(MAIN_PROD)" ;; \
...
build-aab:
	...
	$(FLUTTER) build appbundle --flavor $$FLAVOR -t $$TARGET
...
build-ios:
	...
	$(FLUTTER) build ios --debug --flavor $$FLAVOR -t $$TARGET
...
build-app-split:
	...
	$(FLUTTER) build apk --flavor $$FLAVOR -t $$TARGET --obfuscate --split-debug-info=build/app/outputs/symbols
